...
static constexpr char path_environment[] = "PATH";
static constexpr char env_interpreter[] = "/usr/bin/env";
static constexpr char shebang_prefix[] = "#!";
#if defined(__APPLE__)
static constexpr size_t maximum_shebang_bytes = MAXINTERP;
#else
static constexpr size_t maximum_shebang_bytes = 64;
#endif
static constexpr int signal_exit_status_base = 128;
...
static char *find_executable(const TtrRunner *runner) {
  ...
  for (size_t program_index = 0;
    ...
    if (is_path_like(name)) {
      ...
      const char *configured_path = g_getenv(path_environment);
      g_auto(GStrv) directories =
          g_strsplit(configured_path != nullptr ? configured_path : "",
...
static bool is_javascript_env_shebang(const TtrRuntimeConfig *runtime,
  ...
  char header[maximum_shebang_bytes + 2] = {};
  ...
  if (newline == nullptr && (gsize)count > maximum_shebang_bytes) {
    return false;
  }
  if (newline != nullptr) {
    *newline = '\0';
  }
  if (!g_str_has_prefix(header, shebang_prefix)) {
    return false;
  }
  int argument_count = 0;
  g_auto(GStrv) arguments = nullptr;
  g_autoptr(GError) error = nullptr;
  const bool parsed = g_shell_parse_argv(header + strlen(shebang_prefix),
                                         &argument_count, &arguments, &error);
  const bool matched = parsed && argument_count >= 2 &&
                       g_str_equal(arguments[0], env_interpreter) &&
...
static int spawn_and_wait(const TtrRunner *runner, const char *executable,
  ...
  if (g_subprocess_get_if_signaled(process)) {
    return signal_exit_status_base + g_subprocess_get_term_sig(process);
  }
  return 1;
}
...
int ttr_runner_run(const TtrRunner *runner, const TtrIntegration *integration,
                   ...
                   GError **error) {
  g_return_val_if_fail(runner != nullptr, 1);
