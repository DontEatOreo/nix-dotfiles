...
static bool string_equals(terminal_theme_tools_string value, const char *expected) {
    ...
    return value.length == length && value.data != nullptr && memcmp(value.data, expected, length) == 0;
}
...
int main(void) {
    ...
    assert(terminal_theme_tools_context_create(&options, &context) == TERMINAL_THEME_TOOLS_STATUS_OK);
    ...
    assert(string_equals(terminal_theme_tools_runner_name(context, runner_index), "c-api-shell"));
    assert(string_equals(terminal_theme_tools_runner_program(context, runner_index, 0u), "/bin/sh"));
    ...
    assert(terminal_theme_tools_parse_terminal_report(dark_report, sizeof(dark_report) - 1u) ==
           TERMINAL_THEME_TOOLS_THEME_DARK);
    assert(terminal_theme_tools_parse_terminal_report(light_report, sizeof(light_report) - 1u) ==
           TERMINAL_THEME_TOOLS_THEME_LIGHT);
    assert(terminal_theme_tools_parse_terminal_report(nullptr, 0u) == TERMINAL_THEME_TOOLS_THEME_UNKNOWN);
    ...
    assert(terminal_theme_tools_prepare(
               context,
               "c-api-shell",
               arguments,
    ...
    assert(string_equals(terminal_theme_tools_command_environment(command, "C_API_CHILD"), "configured"));
    assert(terminal_theme_tools_command_temporary_path(command).length == 0u);

    uint8_t exit_code = 0u;
    assert(terminal_theme_tools_command_execute(command, &exit_code) == TERMINAL_THEME_TOOLS_STATUS_OK);
    assert(exit_code == 7u);
